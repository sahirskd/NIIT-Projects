define([], function () {
	...
	CyuMixTemplate.prototype.startPageContent = function () {
		...
		if (window.shell.accessiblityVersion) {
			$(ref.container).find("#assessment .startscreen").css('background-image', 'url(' + + ')');
			$(ref.container).find("#assessment").addClass("accessiblityVersion")
		}
		else {
			$(ref.container).find("#assessment .startscreen").css('background-image', 'url(' + backgroundImgChage + ')');
			$(ref.container).find("#assessment").removeClass("accessiblityVersion")
		}
	...
	CyuMixTemplate.prototype.accessiblitydata = function () {
		...
		var ref = this;
		// 		
		//var topiclenght = $(ref.xml).find("accessiblityPageData").find("topic").length;
		var str = ""
		$.each($(ref.xml).find("accessiblityPageData").find("topic"), function (k, v) {
			str += '<div class="topicDiv">';
			str += '<h1 tabindex="0" class="accessibilityHeading">' + $(v).find("accessiblityHeading").text() + '</h1>';
			str += '<div class="innerContent">';
			str += '<p class="innerPara" tabindex="0">' + $(v).find("accessiblityInnerContent").text() + '</p>';
			str += '</div>';
			str += '<span class="topDivContainer"><button role="button" class="topDiv" tabindex="0">' + $(v).find("gotoTopBtnTxt").text() + '</button></span>';
			str += '</div>';
		})


		$(ref.container).find('.accessiblity-screen-container').find(".topicsdiv").html(str);

		$(ref.container).find('.accessiblity-screen-container').find(".topDiv").bind('click', function (e) {
			$('.scroll_effect').animate({ 'scrollTop': 0 }, 500);
		})
		$(ref.container).find('.takeAssessmentBtn').on('click', function (e) {
			...
			if ($(".carousel-item").length == window.shell.courseProgressArray.length) {
				for (var i = 0; i < $(".carousel-item").length; i++) {
					if ($(".carousel-item").eq(i).hasClass("assessment")) {
						window.shell.courseProgressArray[i] = -1
						window.shell.globalPageVisited[i] = false
					} else {
						window.shell.courseProgressArray[i] = 1
						window.shell.globalPageVisited[i] = true
					}
				}
			}
			$(ref.container).find('#start_screen').show();
			$(ref.container).find('#mainMCQ').hide();
			$(ref.container).find('#accessiblity_container').hide();
			setTimeout(function () {
				$(ref.container).find('#start_screen').find(".screen_inner_containt .subhead").focus()
			}, 150)
		});

		$(ref.container).find('.accessiblity-screen-container').find(".normal").bind('click', function (e) {           
			$(ref.container).find('.accessiblity-screen-container').find(".scroll_effect").removeClass("medium large");
		});


		$(ref.container).find('.accessiblity-screen-container').find(".medium").bind('click', function (e) {
			$(ref.container).find('.accessiblity-screen-container').find(".scroll_effect").addClass("medium").removeClass("normal large");
		})


		$(ref.container).find('.accessiblity-screen-container').find(".large").bind('click', function (e) {
			$(ref.container).find('.accessiblity-screen-container').find(".scroll_effect").addClass("large").removeClass("normal medium");
		})
	}
	...
	CyuMixTemplate.prototype.startScreen = function () {
		...
		if (window.shell.accessiblityVersion) {
			...
			$(ref.container).find('#mainMCQ').hide();
			$(ref.container).find('#accessiblity_container').show();
			setTimeout(function () {
				$(ref.container).find('#accessiblity_container').find(".normal").focus()
			}, 150)
		}
		else {
			$(ref.container).find('#start_screen').show();
		...
		$(ref.container).find('#start-test').on('click', function (e) {
			...
			$("#shellFooter").find(".menuBtn, .btnBack").addClass("disabled")
			if (isLocalfileOrServer != true) {
				window.shell.checkhostReachable()
				if (onLineFlag == "offline") {
					return;
	...
	CyuMixTemplate.prototype.assessmentResultData = function () {
		...
		var resultbgimg = $(ref.xml).find('mcq').find("rerultbackgroundImage").text();
		...
		if (window.shell.accessiblityVersion) {
			$('.cyu_with_context').find(".score-screen").css('background-image', 'url(' + + ')');
		}
		else {
			$('.cyu_with_context').find(".score-screen").css('background-image', 'url(' + resultbgimg + ')');
		}
		...
		if(window.shell.certificateDate!==""){
			$('.cyu_with_context').find("#certificate-screen .current_date").html(window.shell.certificateDate)
		}else{
	...
	CyuMixTemplate.prototype.McqCreateOptions = function () {
		...
		if (window.shell.accessiblityVersion) {
			$(ref.container).find("#assessment #mainMCQ").css('background-image', 'url(' + + ')');
		}
		else {
		...
		var str = '';
		str += '<ul class="cyu-options list-unstyled">';

		for (var i = 0; i < optionLength; i++) {
			str += '<li id="opt_' + (i + 1) + '" class="_item cyu-option" data-index="' + (i + 1) + '">'
			str += '<label class="lab_container" for="radio_' + (i + 1) + '">' + $(ref.xml).find('mcq').find('cyuBlocks').eq(rendomquestionNo).find('options').find('option').eq(i).text();
			str += '<input type="radio" id="radio_' + (i + 1) + '" class="redkBox" name="radio">'
			str += '<span class="radioassessment"></span>'
			str += '</label>'
			str += '</li>'; 
		}
		str += '<div class="clearfix"></div>';
		str += '</ul>';
		str += '<div class="text-left submit-btn-container">' + (ref.isPrevBtn && rendomquestionNo ? '<button class="btn btn-link  prev-btn"><i class="ion-ios-arrow-left"></i>' + ref.prevBtnTxt + '</button>' : '') + '<button tabindex="0" class="btn btn-info cyu-submit-btn disabled" role="button" disabled>Submit<span class="startButton_icon">&nbsp;&#8250;</span></button><div class="feedbackBox"></div></div>';
		// str += '</li>';
		//alert(ref.xmlName)
		//alert($(ref.container))
		$(ref.container).find('#cyu_block').html(str);
		setTimeout(function () {
			$('.cyu_with_context').find(".ques-count").focus()
		}, 150)
	...
	CyuMixTemplate.prototype.MMcqCreateOptions = function () {
		...
		for (var i = 0; i < optionLength; i++) {
			...
			str += '<label class="lab_container" for="checkbox_' + (i + 1) + '">' + $(ref.xml).find('mcq').find('cyuBlocks').eq(rendomquestionNo).find('options').find('option').eq(i).text();
			str += '<input type="checkbox" id="checkbox_' + (i + 1) + '" class="chkBox">'
	...
	CyuMixTemplate.prototype.addMCQFunctionality = function () {
		...
		$(ref.container).find(".cyu-options .cyu-option").find('.redkBox').on("change", function (e) {
			...
			$(ref.container).find(".cyu-submit-btn").removeAttr("disabled");
		});
		...
		$(ref.container).find(".cyu-submit-btn").on('click', function (e) {
			...
			if (ref.selectedOption == ref.correctOption) {
				...
				setTimeout(function () {
					$(ref.container).find(".feedbackBox").find('.correct').find("#correctIncorrect").focus()
				}, 150)
				...
				if (ref.questionCount == (questionShowInaQuize.length - 1)) {
					...
					if ((ref.score / questionShowInaQuize.length).toFixed(2) >= 0.8) {
						...
						window.shell.certificateDate= new Date().toLocaleString('en-us', { month: 'long', year: 'numeric', day: 'numeric' })
						...
						$(ref.container).find('#retry').on('click', function (e) {
							...
							if (window.shell.accessiblityVersion) {
								...
								$(ref.constructor).find("#accessiblity_container").show()
								$(ref.container).find(".cyu-options .cyu-option").removeClass('selected');
							}
							else {
								$(ref.container).find('#start_screen').show();
								$(ref.container).find('.score-screen-fail').hide();
								$(ref.constructor).find("#accessiblity_container").hide()
								$(ref.container).find(".cyu-options .cyu-option").removeClass('selected');
							}
				...
				if (ref.questionCount == (questionShowInaQuize.length - 1)) {
					...
					setTimeout(function () {
						$(ref.container).find(".feedbackBox").find('.inCorrect').find("#correctIncorrect").focus()
					}, 150)
	...
	CyuMixTemplate.prototype.addMMCQFunctionality = function (e) {
		...
		$(ref.container).find(".cyu-options .cyu-option").find('.chkBox').on("change", function (e) {
			...
			if (ref.selectedOptionArray.length == 0) {
				$(ref.container).find(".cyu-submit-btn").addClass("disabled");
				$(ref.container).find(".cyu-submit-btn").attr("disabled");
			}
			else {
				$(ref.container).find(".cyu-submit-btn").removeClass("disabled");
				$(ref.container).find(".cyu-submit-btn").removeAttr("disabled");
	...
	CyuMixTemplate.prototype.createMcqFeedbacks = function () {
		...
		for (var i = 0; i < feedbackLength; i++) {
			...
			str += '<h2 aria-label="" id="correctInc" class="feedback-heading"><img aria-hidden="true" src=' + $(ref.xml).find('cyuBlocks').eq(rendomquestionNo).find('feedbacks').find('feedback').eq(i).attr('icon') + ' style="margin-right: 10px" /> <span tabindex="0" id="correctIncorrect">' + $(ref.xml).find('cyuBlocks').eq(rendomquestionNo).find('feedbacks').find('feedback').eq(i).attr('headerText') + '</span></h2>';
			str += '</div>';
			str += '<button aria-label="" tabindex="0" class="text-center cyu-submit-container ' + $(ref.xml).find('cyuBlocks').eq(rendomquestionNo).find('feedbacks').find('feedback').eq(i).attr('btnclass') + '"><img src="global_files/templates/assessment/images/next_question_load.png" /></button></div>'
			str += '</div></div>';
		}
	...
	CyuMixTemplate.prototype.createMMcqFeedbacks = function () {
		...
		for (var i = 0; i < feedbackLength; i++) {
			str += '<div class="CyufeedbackMainContainer ' + $(ref.xml).find('cyuBlocks').eq(rendomquestionNo).find('feedbacks').find('feedback').eq(i).attr('type') + '">';
			str += '<div class="CyufeedbackContent"><div class="feed-content">';
			str += '<h2 id="correctInc" class="feedback-heading"><img aria-hidden="true" src=' + $(ref.xml).find('cyuBlocks').eq(rendomquestionNo).find('feedbacks').find('feedback').eq(i).attr('icon') + ' style="margin-right: 10px" /><span id="correctIncorrect" tabindex="0">' + $(ref.xml).find('cyuBlocks').eq(rendomquestionNo).find('feedbacks').find('feedback').eq(i).attr('headerText') + '</span></h2>';
			str += '</div>';
			str += '<button class="text-center cyu-submit-container ' + $(ref.xml).find('cyuBlocks').eq(rendomquestionNo).find('feedbacks').find('feedback').eq(i).attr('btnclass') + '"><img src="global_files/templates/assessment/images/next_question_load.png" /></button></div>'
			str += '</div></div>';
		}
		$(ref.container).find('.feedbackBox').html(str);
